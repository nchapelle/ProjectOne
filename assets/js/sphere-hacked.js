...
(function() {
  ...
  var calculateVector = function(h, v) {
    ...
    if (s > 0) {
      ...
      var lh =
        textureWidth +
        (textureWidth * (Math.atan2(L[Y], L[X]) + Math.PI)) / (2 * Math.PI);
      ...
      var lv =
        ...
        Math.floor(
          textureHeight -
            1 -
            ((textureHeight * (Math.acos(L[Z] / r) / Math.PI)) % textureHeight)
        );
      return { lv: lv, lh: lh };
    }
    return null;
  ...
  var sphere = function() {
    ...
    return {
      ...
      init: function(options) {
        ...
        hvs = 0.5 * vs; // half vertical scale // vector for storing direction of each pixel from F

        /*V = new Array(3);*/ L = new Array(3); // Location vector from S that pixel 'hits' sphere
      ...
      RF: function(time) {
        ...
        while (pixel--) {
          ...
          if (vector !== null) {
            ...
            var lh =
              Math.floor(
                vector.lh * tiling.horizontal + turnBy * tiling.horizontal
              ) % textureWidth;
            /*           lh = (lh < 0)
