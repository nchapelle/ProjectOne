...
(function($) {
  $.widget("ui.earth3d", {
    options: {
      texture: "assets/img/earth1024x1024.jpg",
      sphere: {
        tilt: 0,
        turn: 0,
        r: 10
      ...
      onRefreshFlight: function(flight, x, y, angle, widget) {
        flight.$element.css({
          ...
          top: y,
          "-webkit-transform":
            "rotate(" + ((angle + Math.PI / 2) * 360) / (2 * Math.PI) + "deg)",
          "-moz-transform":
            "rotate(" + ((angle + Math.PI / 2) * 360) / (2 * Math.PI) + "deg)",
          "-o-transform":
            "rotate(" + ((angle + Math.PI / 2) * 360) / (2 * Math.PI) + "deg)"
    ...
    _create: function() {
      ...
      createSphere(
        ...
        function(earth, textureWidth, textureHeight) {
    ...
    _onSphereCreated: function(earth, textureWidth, textureHeight) {
      ...
      var renderAnimationFrame = function(/* time */ time) {
        ...
        self.renderAnimationFrameId = window.requestAnimationFrame(
    ...
    _renderAnimationFrame: function(time) {
      ...
      for (var key in this.options.locations) {
        ...
        if (typeof location.delta === "undefined") {
          ...
          this.options.onRefreshLocation(
            location,
            location.x,
            location.y,
            this
        ...
        * */
        var progression =
          ((this.posVar +
            (this.textureWidth * location.delta) / (2 * Math.PI)) %
            this.textureWidth) /
          this.textureWidth;
        var alphaAngle = progression * 2 * Math.PI;
        var deltaAngle =
          this._calibrated(progression, location.alpha) * 2 * Math.PI;

        var objAlpha =
          ry +
          location.alpha -
          (Math.sin(alphaAngle / 2) * 0.15 * (location.alpha - Math.PI / 2)) /
            (Math.PI / 4);
      ...
      if (this.flightsCtx !== null) {
        this.flightsCtx.clearRect(
          0,
          0,
          this.options.flightsCanvas.width(),
          this.options.flightsCanvas.height()
        );
        for (var key in this.options.paths) {
    ...
    _drawPath: function(path, center, r) {
      ...
      if (
        typeof originLocation.delta === "undefined" ||
        typeof destinationLocation.delta === "undefined"
      ) {
        var pathVisible = originLocation.visible && destinationLocation.visible;
        if (pathVisible) {
          var flatDistance = this._distance(
            originLocation.flatPosition,
            destinationLocation.flatPosition
          );
          ...
          for (var i = 0; i < nb; i++) {
            var fromFlatPosition = {
              x:
                ((nb - i) / nb) * originLocation.flatPosition.x +
                (i / nb) * destinationLocation.flatPosition.x,
              y:
                ((nb - i) / nb) * originLocation.flatPosition.y +
                (i / nb) * destinationLocation.flatPosition.y
            };

            var toFlatPosition = {
              x:
                Math.max((nb - (i + 1)) / nb, 0) *
                  originLocation.flatPosition.x +
                Math.min((i + 1) / nb, 1) * destinationLocation.flatPosition.x,
              y:
                Math.max((nb - (i + 1)) / nb, 0) *
                  originLocation.flatPosition.y +
                Math.min((i + 1) / nb, 1) * destinationLocation.flatPosition.y
            };
            ...
            this.flightsCtx.moveTo(
              fromFlatPosition.x + this.options.flightsCanvasPosition.x,
              fromFlatPosition.y + this.options.flightsCanvasPosition.y
            );
            this.flightsCtx.lineTo(
              toFlatPosition.x + this.options.flightsCanvasPosition.x,
              toFlatPosition.y + this.options.flightsCanvasPosition.y
            );
        ...
        for (var key in path.flights) {
          ...
          var position =
            flight.destination == path.destination
              ? flight.position
              : 1 - flight.position;

          var flightFlatPosition = {
            x:
              (1 - position) * originLocation.flatPosition.x +
              position * destinationLocation.flatPosition.x,
            y:
              (1 - position) * originLocation.flatPosition.y +
              position * destinationLocation.flatPosition.y
          };
          ...
          var angle =
            Math.atan2(
              destinationLocation.flatPosition.y -
                originLocation.flatPosition.y,
              destinationLocation.flatPosition.x - originLocation.flatPosition.x
            ) + (flight.destination == path.destination ? 0 : Math.PI);
          //console.log(flightAheadFlatPosition.y - flightFlatPosition.y);

          this.options.onRefreshFlight(
            flight,
            flightFlatPosition.x,
            flightFlatPosition.y,
            angle,
            this
          );
        }
      ...
      var locationsDistance = this._distance(
      ...
      var originP = {
        delta: Math.atan2(
          originLocation.position.y - middlePosition.y,
          originLocation.position.x - middlePosition.x
        ),
        alpha: Math.acos(
          (originLocation.position.z - middlePosition.z) / radius
        )
      };

      var destinationP = {
        delta: Math.atan2(
          destinationLocation.position.y - middlePosition.y,
          destinationLocation.position.x - middlePosition.x
        ),
        alpha: Math.acos(
          (destinationLocation.position.z - middlePosition.z) / radius
        )
      };
      ...
      if (!path.nb) {
        path.nb = Math.round(
          ((locationsDistance / (2 * r)) * Math.PI * 2 * r +
            (1 - locationsDistance / (2 * r)) * locationsDistance) /
            dotSize
        );
      }
      var nb = path.nb;
      ...
      for (var i = 0; i < nb; i++) {
        var fromP = {
          alpha:
            ((nb - i) / nb) * originP.alpha + (i / nb) * destinationP.alpha,
          delta: ((nb - i) / nb) * originP.delta + (i / nb) * destinationP.delta
        };

        var toP = {
          alpha:
            ((nb - 1 - i) / nb) * originP.alpha +
            ((i + 1) / nb) * destinationP.alpha,
          delta:
            ((nb - 1 - i) / nb) * originP.delta +
            ((i + 1) / nb) * destinationP.delta
        };
        //console.log(i, fromP.alpha, fromP.delta, toP.alpha, toP.delta);

        var fromPosition = this._orbitalTo3d(
          fromP.alpha,
          fromP.delta,
          -(Math.sin((Math.PI * i) / nb) * (maxDistance - 1) + 1) * radius
        );
        var toPosition = this._orbitalTo3d(
          toP.alpha,
          toP.delta,
          -(Math.sin((Math.PI * (i + 1)) / nb) * (maxDistance - 1) + 1) * radius
        );
        var diff = {
        ...
        if (!fromVisible) {
          var intersection = this._line_circle_intersection(
            fromFlatPosition,
            toFlatPosition,
            center,
            r
      ...
      for (var key in path.flights) {
        ...
        var positionAhead =
          flight.destination == path.destination
            ? flight.position + 0.01
            : 1 - (flight.position + 0.01);
        ...
        var flightAheadP = {
          alpha:
            (1 - positionAhead) * originP.alpha +
            positionAhead * destinationP.alpha,
          delta:
            (1 - positionAhead) * originP.delta +
            positionAhead * destinationP.delta
        };

        var flightPosition = this._orbitalTo3d(
          flightP.alpha,
          flightP.delta,
          -(Math.sin(Math.PI * position) * (maxDistance - 1) + 1) * radius
        );
        var flightAheadPosition = this._orbitalTo3d(
          flightAheadP.alpha,
          flightAheadP.delta,
          -(Math.sin(Math.PI * positionAhead) * (maxDistance - 1) + 1) * radius
        );
        ...
        var flightAheadFlatPosition = this._orthographicProjection(
          flightAheadPosition
        );
        ...
        if (
          !flight.visible &&
          (flightPosition.x < 0 || flightDistanceCenter > r)
        ) {
          this.options.onShowFlight(flight, this);
          flight.visible = true;
        }
        ...
        var angle = Math.atan2(
          flightAheadFlatPosition.y - flightFlatPosition.y,
          flightAheadFlatPosition.x - flightFlatPosition.x
        );
        //console.log(flightAheadFlatPosition.y - flightFlatPosition.y);
    ...
    _calibrated: function(x, alpha) {
      ...
      //console.log(calib);
      var y =
        calib * (4 * (x - 0.5) * (x - 0.5) * (x - 0.5) + 0.5) + (1 - calib) * x;
      return y;
    },
    ...
    _3dProjection: function(a, c, delta, e) {
      ...
      d.x =
        Math.cos(delta.y) *
          (Math.sin(delta.z) * (a.y - c.y) + Math.cos(delta.z) * (a.x - c.x)) -
        Math.sin(delta.y) * (a.z - c.z);
      d.y =
        Math.sin(delta.x) *
          (Math.cos(delta.y) * (a.z - c.z) +
            Math.sin(delta.y) *
              (Math.sin(delta.z) * (a.y - c.y) +
                Math.cos(delta.z) * (a.x - c.x))) +
        Math.cos(delta.x) *
          (Math.cos(delta.z) * (a.y - c.y) - Math.sin(delta.z) * (a.x - c.x));
      d.z =
        Math.cos(delta.x) *
          (Math.cos(delta.y) * (a.z - c.z) +
            Math.sin(delta.y) *
      ...
      if (this.lastMousePos === null) {
        if (this.lastSpeed !== null) {
          ...
          if (this.options.backToDefaultTime + this.lastTime - time < 0) {
            ...
            var backToDef =
              (this.options.backToDefaultTime + this.lastTime - time) /
              this.options.backToDefaultTime;
            this.posVar -=
              this.lastSpeed * backToDef +
              this.options.defaultSpeed * timeDiff * (1 - backToDef);
          }
        } else {
    ...
    _getQuadraticCurvePoint: function(
      startX,
      startY,
      cpX,
      cpY,
